...
#####################################################################################################################
max_iters = int(2e2)
term_cond = 8000 # Terminal confition, difference of 'descent' direction
...
epsilon = 1000  #Epsilon per la barrier function, viene lasciata costatne
...
if Curve == 'Step'  or  Curve == 'Sigmoid':
  ...
  ######################################
  # Physical constraints over inputs
  ######################################
  uu_limit = np.zeros(ni)  
  uu_limit[0] = 0.55
  uu_limit[1] = 800
...
for kk in range(max_iters-1):
  ...
  # Se lo riduco, rimene in stallo le le prime iterazione aspettando che si riduca per poi ottenere lo stesso risultato senza barrier function
  # epsilon = (epsilon /5)
  ...
  for tt in range(TT-1):
    temp_cost = cst.stagecost_barrier_function(xx[:,tt, kk], uu[:,tt,kk], xx_ref[:,tt], uu_ref[:,tt], QQt, RRt, uu_limit, epsilon)[0]
    JJ[kk] += temp_cost
  ...
  ## ##################################################################
  for tt in range(TT):
    aa, bb = cst.stagecost_barrier_function(xx[:,tt, kk], uu[:,tt,kk], xx_ref[:,tt], uu_ref[:,tt], QQt, RRt, uu_limit, epsilon)[1:] # aa -> (6,1) , bb -> (2,1)
  ...
  if Do_Armijo:
    ...
    for Iter_Armijo in range(armijo_maxiters):
      ...
      JJ_temp = 0
      for tt in range(TT-1):
        temp_cost = cst.stagecost_barrier_function(xx_temp[:,tt], uu_temp[:,tt], xx_ref[:,tt], uu_ref[:,tt], QQt, RRt, uu_limit, epsilon)[0]
        JJ_temp += temp_cost
  ...
  if np.linalg.norm(descent[kk] - descent[kk-1]) <= term_cond:
...
for ii in range(0,int(ns/2)):
  ...
  # axs[ii].plot(tt_hor, xx_init[ii,:], 'k', linewidth=2, label='$Init$') #Non necessaria nel task 5
  axs[ii].plot(tt_hor, xx_star[ii,:], 'r', linewidth=2, label='$Opt$')
  axs[ii].grid()
  axs[ii].set_ylabel('$x_{}$'.format(ii))
axs[ii].set_xlabel('time')
fig_X_1.legend(['$Ref$', '$Opt$'], loc='lower right', ncol = 2, fontsize = 10) #'$Init$',
...
fig_X_2.legend(['$Ref$', '$Opt$'], loc='lower right', ncol = 2, fontsize = 10)
fig_X_2.suptitle('Evolution of State [3,4,5]')
plt.show(block=False)

# Inserito il plot della saturazione massima e minima
uu_lim_plot = np.zeros((ni, TT))
uu_lim_plot[0,:] = uu_limit[0]
uu_lim_plot[1,:] = uu_limit[1]
fig_U, axs = plt.subplots(ni, 1, sharex='all')
axs[0].plot(tt_hor, uu_ref[0,:], 'g--', linewidth=2, label='$Ref$')
axs[0].plot(tt_hor, uu_star[0,:], 'r', linewidth=2, label='$Opt$')
# axs[0].plot(tt_hor, uu_lim_plot[0,:], 'm-.', linewidth=2, label='$Limit$')
axs[0].plot(tt_hor, -uu_lim_plot[0,:], 'm-.', linewidth=2, label='$Limit$')
axs[0].grid()
axs[0].set_ylabel('$u_{}$'.format(0))
axs[1].plot(tt_hor, uu_ref[1,:], 'g--', linewidth=2, label='$Ref$')
axs[1].plot(tt_hor, uu_star[1,:], 'r', linewidth=2, label='$Opt$')
axs[1].plot(tt_hor, uu_lim_plot[1,:], 'm-.', linewidth=2, label='$Limit$')
# axs[1].plot(tt_hor, -uu_lim_plot[1,:], 'm-.', linewidth=2, label='$Limit$')
axs[1].grid()
axs[1].set_ylabel('$u_{}$'.format(1))

axs[1].set_xlabel('time')
fig_U.legend(['$Ref$', '$Opt$', '$Limit$'], loc='lower right', ncol = 3, fontsize = 10)
